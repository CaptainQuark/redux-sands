...
exports.default = class ReduxWrapper {
  ...
  mapStateToProps(state) {
    let mapped = _extends({}, state[this._id]);
    const props = _extends({}, this.otherStateProps);
    ...
    Object.keys(props).forEach(id => {
      if (props[id] === "all") {
        return mapped = _extends({}, mapped, state[id]);
      }

      if (props[id].constructor === Array) {
        const otherCopy = _extends({}, state[id]);
  ...
  add(props) {
    ...
    if (initState) this.initState = initState;else if (component) this.component = component;else if (reducer) this._addActionReducer({ action: reducer });else if (otherStateProps) this.otherStateProps = _extends({}, this.otherStateProps, { otherStateProps });
  ...
  dispatches(dispatch) {
    ...
    Object.keys(actionReducers).forEach(type => {
      obj = _extends({}, obj, this._dispatch({ dispatch, type, action: actionReducers[type] }));
    ...
    Object.keys(this.otherStateActions).forEach(wrapperName => {
      ...
      this.otherStateActions[wrapperName].forEach(element => {
        ...
        obj[names.exposedName] = params => dispatch(_extends({ type: t.proxy, dispatch }, params));
      });
    });
  ...
  get connection() {
    return connect(this.mapStateToProps, this.mapDispatchToProps)(this.component);
  }
  ...
  reducer(state = this.initState, action) {
    ...
    if (this.actionReducers[action.type] && this.actionReducers[action.type].isReducable) return this.actionReducers[action.type].f(state, action);
  ...
  _dispatch(props) {
    ...
    return {
      [_action.name]: params => _action.params ? dispatch(_extends({ type }, params, _action.params)) : dispatch(_extends({ type }, params))
    };
  }

  _importOtherDispatchProps(props) {
    const copy = _extends({}, props.reducer);
    ...
    Object.keys(copy).forEach(wrapperName => {
      ...
      copy[wrapperName].forEach(tuple => {
        ...
        this.saga.push(takeEvery(t.proxy, function* (action) {
          const copy = _extends({}, action);
          const { dispatch } = copy;
          let foreignParams = {};

          Object.keys(action).filter(key => key !== "dispatch" && key !== "type").forEach(key => foreignParams[key] = copy[key]);
          ...
          withSaga ? yield put(_extends({ type: t.saga.req, call, put, result: { type: t.saga.rec } }, foreignParams)) : yield put(_extends({ type: t.basic }, foreignParams));
        }));
      });
  ...
  _importOtherStateProps(props) {
    const stateCopy = _extends({}, props.state);
    Object.keys(stateCopy).forEach(id => this.otherStateProps = _extends({}, this.otherStateProps, { [id]: stateCopy[id] }));
  }

  _addActionReducer({ action }) {
    if (action[Object.keys(action)[0]].withSaga) return this._addSaga(action);else this._addAtomicActionReducer({ action, isShort: !action[Object.keys(action)[0]].fn });
  }
  ...
  static _createExternalTypes({ namespace, fName, _id, withSaga = false }) {
    ...
    if (withSaga) tuples.saga = {
      req: `SAGA_REQ_${namespace.toUpperCase()}_${fName.toUpperCase()}`,
      rec: `SAGA_REC_${namespace.toUpperCase()}_${fName.toUpperCase()}`
    };

    return _extends({}, tuples);
  }

  static _extractImportReducerParams(element) {
    let names = {};
    if (element.constructor === String) names = { fName: element, exposedName: element, withSaga: false };else if (element.constructor === Object) {
